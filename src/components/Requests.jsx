...
const Requests = () => {
  ...
  return (  
    <div className='text-center my-10'>
        <h1 className='text-bold text-white text-3 xl'>Connection Requests</h1>
        {requests.map((request) => { 
            const {_id, firstName, lastName, photoUrl, age, gender, about} =
             request.fromUserId;
            
        return (
        <div key={_id} className='flex justify-between items-center m-4 p-4 border rounded-lg bg-base-300 mx-auto'>
            ...
            <div>
                <button className="btn btn-soft btn-accent mx-2" 
                    onClick={()=> reviewRequest("rejectetd", request._id)}>
                    Reject
                </button>
                <button className="btn btn-soft btn-info mx-2" 
